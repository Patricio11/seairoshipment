...
import { destinationCharges } from "@/lib/db/schema";
import { eq } from "drizzle-orm";
import { parseBulkIds, runBulkDelete } from "@/lib/bulk-delete";

/** Items cascade-delete via FK; no other dependents to guard against. */
export async function POST(req: NextRequest) {
    ...
    const result = await runBulkDelete(parsed.ids, async (id) => {
        const [deleted] = await db.delete(destinationCharges).where(eq(destinationCharges.id, id)).returning();
