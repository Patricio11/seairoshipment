...
import { UploadCloud, FileText, Loader2, Shield } from "lucide-react"
import {
    Dialog,
    DialogContent,
    DialogDescription,
...
interface AdminUploadDialogProps {
    /** The client allocation to attach the doc to */
    allocationId: string
    /** Booking ref for the dialog header */
    bookingRef?: string | null
    /** Client name so the admin sees whose booking they're uploading to */
    clientName?: string | null
    /** Fired after a successful upload so the parent can refresh its doc list */
    onUploaded?: () => void
    /** Optional custom trigger; defaults to a small "Upload" button */
...
export function AdminUploadDialog({
    ...
    const handleUpload = async () => {
        ...
        try {
            ...
            const res = await fetch(`/api/admin/allocations/${allocationId}/upload`, {
                method: "POST",
                body: fd,
            })
            ...
            toast.success("Document uploaded for client", {
                description: `${file.name}${clientName ? ` · ${clientName}` : ""}`,
            })
            reset()
            setIsOpen(false)
            onUploaded?.()
                    ...
                    >
                        ...
                        {file ? (
                            ...
                            <>
                                ...
                                <p className="text-xs text-slate-400 mt-1">PDF / Excel / JPG / PNG</p>
                            </>
                        )}
                    </label>
                </div>
