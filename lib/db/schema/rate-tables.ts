...
export const originChargeItems = pgTable("origin_charge_items", {
    ...
    buyUnitCost: numeric("buy_unit_cost"),
    buyContainerCost: numeric("buy_container_cost"),
...
export const oceanFreightRates = pgTable("ocean_freight_rates", {
    ...
    buyFreightUSD: numeric("buy_freight_usd").default("0").notNull(),
    buyBafUSD: numeric("buy_baf_usd").default("0").notNull(),
    buyIspsUSD: numeric("buy_isps_usd").default("0").notNull(),
    buyOtherSurchargesUSD: numeric("buy_other_surcharges_usd").default("0").notNull(),
    buyRcgUSD: numeric("buy_rcg_usd").default("0").notNull(),
    buyTotalUSD: numeric("buy_total_usd").default("0").notNull(),
    buyTotalZAR: numeric("buy_total_zar").default("0").notNull(),
    active: boolean("active").default(true).notNull(),
    createdAt: timestamp("created_at").defaultNow().notNull(),
    updatedAt: timestamp("updated_at").defaultNow().notNull(),
});
...
export const destinationChargeItems = pgTable("destination_charge_items", {
    ...
    buyAmountZAR: numeric("buy_amount_zar"),
    sortOrder: integer("sort_order").default(0).notNull(),
    notes: text("notes"),
    createdAt: timestamp("created_at").defaultNow().notNull(),
    updatedAt: timestamp("updated_at").defaultNow().notNull(),
